...
Select @@ServerName[Server Name],name,'',sysadmin,securityadmin,serveradmin,setupadmin,processadmin,diskadmin,dbcreator,bulkadmin from master..syslogins where (name <> 'sa' and name not like '##%' and name not like '%DLG_SQL_%' and name not like 'NT %')

-- Output Job Information
select @@ServerName[Server Name],'',name,enabled from msdb..sysjobs where name not like 'DBA_Admin%' and name not like 'Raise Tivoli%' and name <> 'syspolicy_purge_history' and name not like 'IBM DBA%' and name not like 'SQL Sentry%'
